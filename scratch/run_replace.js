...
// A. Inject code blocks after declaration
const injectLineIndex = lines.findIndex(line => line.includes('let activeChannelIndex = 0;'));
if (injectLineIndex === -1) {
    console.error('Marker "let activeChannelIndex = 0;" not found');
    process.exit(1);
}
lines.splice(injectLineIndex + 1, 0, tabSwitchingCode + sparklineCode + matchAnalyticsCode);

// Re-join because indexing has shifted, and we can find other blocks dynamically
let newHtml = lines.join('\n');

// B. Replace renderChannelList
const startChannelListIndex = newHtml.indexOf('function renderChannelList() {');
const endChannelListIndex = newHtml.indexOf('// Register Shaka loader event');
if (startChannelListIndex === -1 || endChannelListIndex === -1 || startChannelListIndex >= endChannelListIndex) {
    console.error('Could not identify boundaries of renderChannelList');
    process.exit(1);
}
newHtml = newHtml.substring(0, startChannelListIndex) + replacementChannelListRender + '\n\n        ' + newHtml.substring(endChannelListIndex);

// C. Replace filterUpcomingMatches
const startFilterUpcomingIndex = newHtml.indexOf('window.filterUpcomingMatches = function() {');
const endFilterUpcomingIndex = newHtml.indexOf('window.activeStandingsGroup = \'A\';');
if (startFilterUpcomingIndex === -1 || endFilterUpcomingIndex === -1 || startFilterUpcomingIndex >= endFilterUpcomingIndex) {
    console.error('Could not identify boundaries of filterUpcomingMatches');
    process.exit(1);
}
newHtml = newHtml.substring(0, startFilterUpcomingIndex) + replacementFilterMatches + '\n\n        ' + newHtml.substring(endFilterUpcomingIndex);

// D. Replace renderUpcomingTable
const startRenderUpcomingIndex = newHtml.indexOf('function renderUpcomingTable(upcomingList) {');
const endRenderUpcomingIndex = newHtml.indexOf('// Live Realtime calculations updates loop');
if (startRenderUpcomingIndex === -1 || endRenderUpcomingIndex === -1 || startRenderUpcomingIndex >= endRenderUpcomingIndex) {
    console.error('Could not identify boundaries of renderUpcomingTable');
    process.exit(1);
}
newHtml = newHtml.substring(0, startRenderUpcomingIndex) + replacementRenderUpcomingTable + '\n\n        ' + newHtml.substring(endRenderUpcomingIndex);

// E. Replace DOMContentLoaded content
const searchDOMEvaluator = `            fetchSportScoreMatches();
            // Refresh scores every 45 seconds
            setInterval(fetchSportScoreMatches, 45000);
...
if (!newHtml.includes(searchDOMEvaluator)) {
    console.error('Original DOMContentLoaded listener block not found');
    process.exit(1);
}
newHtml = newHtml.replace(searchDOMEvaluator, replacementDOMContentLoaded);

fs.writeFileSync(filePath, newHtml, 'utf8');
console.log('Successfully completed line-based JS updates on index.html!');
